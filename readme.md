...
- Integrates with dbt Core via Cosmos for analytics transformations
- Provides visibility into pipeline success/failures

### Ingestion from Synthea
...
- Database engine for healthcare data transformations
- Provisioned and configured via Terraform
- Optimized for analytical queries on healthcare datasets

### Analytics Transformations - dbt Core
- Runs within Airflow using Cosmos integration
- Implements a three-layer transformation approach:
  - **Staging**: Data type conversions and validations
  - **Intermediate**: In-row transformations and calculations
  - **Marts**: Analytics-ready dimensional models for healthcare analytics

### Visualization - Looker Studio
- Interactive dashboards for healthcare analytics

## dbt Data Lineage
The project follows a modular transformation pattern with clear data lineage:
...
Intermediate
- In-row transformations, calculations
...
- `dim_providers`: Healthcare providers information
- `dim_organizations`: Healthcare facilities information
- `dim_conditions`: Standardized medical conditions and diagnoses, using SNOMED CT
- `dim_medications`: Pharmaceutical products and prescribed medications
...
- BigQuery datasets for staging, intermediate, and production data
